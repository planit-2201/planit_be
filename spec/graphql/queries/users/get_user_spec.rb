...
RSpec.describe Types::QueryType do
  before :each do
    ...
    dates[0..9].each {|date| FactoryBot.create(:daily_record, user_id: @user.id, date: date, shower_time: 200, bottle_count: 3 )}
    dates[10..19].each {|date| FactoryBot.create(:daily_record, user_id: @user.id, date: date, shower_time: 300, bottle_count: 4 )}
    dates[20..29].each {|date| FactoryBot.create(:daily_record, user_id: @user.id, date: date, shower_time: 400, bottle_count: 5 )}
