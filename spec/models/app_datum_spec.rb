...
RSpec.describe AppDatum, type: :model do
   describe 'instance methods' do
    before :each do
      ...
      @users.each do |user|
        create(:daily_record, user_id: user.id, date: dates[0], shower_time: 100, straw_count: 1, bag_count: 6)
        create(:daily_record, user_id: user.id, date: dates[10], shower_time: 200, straw_count: 3, bag_count: 2)
        create(:daily_record, user_id: user.id, date: dates[20], shower_time: 300, straw_count: 5, bag_count: 4)
      end
